        ...
        function render(s, err) {
          ...
          if (s) {
            ...
            for (var i = 0; i < (s.devices || []).length; i++) {
              ...
              rows.push('<div class="hdcp-dev' + (isPref ? ' hdcp-dev-pref' : '') + '" data-id="' + d.id + '" title="点击设为默认设备"><button class="hdcp-dev-shot" data-shot="' + d.id + '" title="截此设备">截图</button><div class="hdcp-dev-main"><div class="hdcp-dev-name">' + nameLine + '</div><div class="hdcp-dev-sub">' + subBadges.join('') + '</div></div></div>');
            }
