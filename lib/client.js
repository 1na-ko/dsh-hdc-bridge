...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      '.hdcp-entry-dot{position:absolute;top:2px;right:2px;width:8px;height:8px;border-radius:50%;border:2px solid var(--dsw-alias-bg-base,#f9fafb);background:var(--dsw-alias-label-quaternary,#8a8f99);}',
      ...
      // ---- modal overlay (portal to body; same layering as the remote plugin) ----
      '.hdcp-overlay{z-index:1000;justify-content:center;align-items:center;display:flex;position:fixed;inset:0;}',
      '.hdcp-mask{background:var(--dsw-alias-bg-mask-1,rgba(0,0,0,.3));backdrop-filter:var(--dsw-mask-blur,blur(2px));position:absolute;inset:0;}',
      '.hdcp-root{position:relative;width:380px;max-width:calc(100vw - 32px);max-height:calc(100vh - 48px);pointer-events:auto;font:12px/1.5 system-ui,-apple-system,Segoe UI,sans-serif;color:var(--dsw-alias-label-primary,#e9e9ef);}',
      '.hdcp-card{background:var(--dsw-alias-bg-layer-3,#23252c);border:1px solid var(--dsw-alias-border-l2,#3a3d46);border-radius:12px;box-shadow:0 12px 32px rgba(0,0,0,.45);display:flex;flex-direction:column;position:relative;overflow:hidden;}',
    ...
    function PanelSurface(props) {
      ...
      return React.createElement('div', { className: 'hdcp-root', ref: rootRef },
        React.createElement('div', { className: 'hdcp-card' },
          ...
          React.createElement('div', { className: 'hdcp-body' }, bodyKids),
          handles
        )
      );
    }
    ...
    function EntryButton(props) {
      ...
      return React.createElement(React.Fragment, null,
        ...
        open ? ReactDOM.createPortal(
          React.createElement('div', { className: 'hdcp-overlay', role: 'presentation' },
            React.createElement('div', { className: 'hdcp-mask', 'aria-hidden': 'true', onClick: close }),
            React.createElement(PanelSurface, { data: data, onClose: close })
          ),
          document.body
        ) : null
      );
