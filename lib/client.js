...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      // ROW (our entry sat beside the remote-control button, and the row's
      // stretch alignment pulled our 36px circle into an oval). The slot
      // renderer wraps entries in a display:contents layer, so target the row
      // itself (grandparent of our host) and stack it vertically; keep the
      // host centered so the button keeps its circle shape.
      '.hdcp-entry-host{display:flex;justify-content:center;align-items:center;width:100%;}',
      '*:has(> div:has(> .hdcp-entry-host)){flex-direction:column;align-items:center;}',
    ...
    function EntryButton(props) {
      ...
      return React.createElement('div', { className: 'hdcp-entry-host' },
