    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        function schedule() {
          ...
          var busy = state && state.devices && state.devices.length > 0;
          timer = setInterval(poll, busy ? POLL_FAST : POLL_SLOW);
        }
        poll().then(schedule);
        return () => { if (timer) clearInterval(timer); if (root.parentNode) root.parentNode.removeChild(root); };
