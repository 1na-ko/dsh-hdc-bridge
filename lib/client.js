...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      '#hdc-panel-root .hdcp-body{padding:10px;border-top:1px solid var(--dsw-alias-border-l2,#3a3d46);flex:1 1 auto;overflow:auto;}',
      ...
      '#hdc-panel-root .hdcp-pre{max-height:140px;overflow:auto;background:#16171c;padding:6px;border-radius:6px;font:10px/1.4 ui-monospace,Consolas,monospace;margin:8px 0 0;white-space:pre-wrap;word-break:break-all;}',
      ...
      '#hdc-panel-root .hdcp-resize{position:absolute;right:0;bottom:0;width:16px;height:16px;cursor:nwse-resize;touch-action:none;background:linear-gradient(135deg,transparent 50%,var(--dsw-alias-border-l2,#3a3d46) 50%);}',
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        root.innerHTML =
          ...
          '</div>' +
          '<div class="hdcp-resize" id="hdcp-resize"></div>' +
          '</div>';
        ...
        var resizeHandle = root.querySelector('#hdcp-resize');
        ...
        resizeHandle.addEventListener('pointerdown', function (e) {
          resizeInfo = { sx: e.clientX, sy: e.clientY, w: root.offsetWidth, h: root.offsetHeight };
          try { resizeHandle.setPointerCapture(e.pointerId); } catch (err) { /* older browsers */ }
          e.preventDefault();
        });
        resizeHandle.addEventListener('pointermove', function (e) {
          if (!resizeInfo) return;
          var w = Math.max(240, resizeInfo.w + (e.clientX - resizeInfo.sx));
          var h = Math.max(160, resizeInfo.h + (e.clientY - resizeInfo.sy));
          root.style.width = w + 'px';
          root.style.height = h + 'px';
          resizedH = h + 'px';
        });
        resizeHandle.addEventListener('pointerup', function () { resizeInfo = null; });
        resizeHandle.addEventListener('pointercancel', function () { resizeInfo = null; });
