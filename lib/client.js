  ...
  factory: (require) => {
    ...
    function EntryButton(props) {
      ...
      React.useEffect(function () {
        ...
        // between). Add our own class to it so the row stacks vertically; the
        // cleanup removes it when the platform unloads this entry.
        var row = host.parentElement && host.parentElement.parentElement;
        if (row) {
          row.classList.add('hdcp-footer-stack');
