...
// Hand-written in the client module format (lazy CJS factory registered
// through window.__ModuleLoader__) like the official client bundles — no
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      '#hdc-panel-root{position:fixed;top:70px;right:16px;z-index:2147483000;width:302px;font:12px/1.5 system-ui,-apple-system,Segoe UI,sans-serif;color:var(--dsw-alias-text-primary,#e9e9ef);}',
      '#hdc-panel-root .hdcp-card{background:var(--dsw-alias-bg-layer-3,#23252c);border:1px solid var(--dsw-alias-border-l2,#3a3d46);border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,.38);overflow:hidden;}',
      '#hdc-panel-root .hdcp-head{display:flex;align-items:center;gap:8px;padding:8px 10px;}',
      ...
      '#hdc-panel-root .hdcp-body{padding:10px;border-top:1px solid var(--dsw-alias-border-l2,#3a3d46);}',
      ...
      '#hdc-panel-root .hdcp-shot{margin-top:8px;}',
        ...
        function render(s, err) {
          ...
          if (s) {
            ...
            if (s.screenshot && s.screenshot.available) {
              ...
              if (!img) { img = document.createElement('img'); shotBox.appendChild(img); }
