...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      '#hdc-panel-root .hdcp-dev{padding:4px 0;border-bottom:1px dashed var(--dsw-alias-border-l2,#2c2e37);display:flex;align-items:center;gap:6px;cursor:pointer;border-radius:4px;}',
        ...
        function render(s, err) {
          ...
          if (s) {
            ...
            for (var i = 0; i < (s.devices || []).length; i++) {
              ...
              var badge = (isPref ? '<span class="hdcp-badge hdcp-badge-pref">默认</span>' : '') + (d.model ? '<span class="hdcp-badge">' + d.model + (d.apiVersion ? ' · API ' + d.apiVersion : '') + '</span>' : (d.apiVersion ? '<span class="hdcp-badge">API ' + d.apiVersion + '</span>' : ''));
              rows.push('<div class="hdcp-dev' + (isPref ? ' hdcp-dev-pref' : '') + '" data-id="' + d.id + '" title="点击设为默认设备"><span class="hdcp-dev-name">' + d.id + '</span>' + badge + '</div>');
            }
