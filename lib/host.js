...
export function apply(ctx) {
  ...
  async function ensureCli(policy) {
    ...
    if (pkg.ok) {
      try {
        const r = await runShellRaw(devcli.buildCliCommand(pkg, ['--version'], psQuote), 30000, 8192, policy)
        if (r.exitCode === 0) {
  ...
  async function runCli(cli, argv, opts, policy) {
    const cmd = devcli.buildCliCommand(cli, argv, psQuote)
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'build' || args.action === 'clean') {
        ...
        if (cli.ok) {
          ...
          return { ...outside, backend: 'devecocli', ok: r.ok, exitCode: r.exitCode, timedOut: r.timedOut, output: tailText((r.stdout + '\n' + r.stderr).trim(), 8000), error: r.ok ? '' : (r.stderr || r.stdout) }
        }
        const hvigor = st.ok ? studio.hvigorwPath(st.root) : { ok: false, path: '', kind: '' }
        if (!hvigor.ok || !sdk.ok) {
        ...
        const cmd = shellFlavor === 'pwsh'
          ? '$env:DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + '; & ' + psQuote(hvigor.path) + ' assembleHap --mode module -p product=' + product + ' -p buildMode=' + mode + ' --no-daemon'
          : 'DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + ' ' + psQuote(hvigor.path) + ' assembleHap --mode module -p product=' + product + ' -p buildMode=' + mode + ' --no-daemon'
        const r = await runShellRaw(cmd, timeoutMs, 1048576, policy)
        const out = (r.stdout && r.stdout.text) || ''
        const err = (r.stderr && r.stderr.text) || ''
        return { ...outside, backend: 'hvigorw', ok: r.exitCode === 0 && !r.timedOut, exitCode: r.exitCode, timedOut: r.timedOut === true, output: tailText((out + '\n' + err).trim(), 8000), note2: 'hvigorw uses the signingConfigs already present in build-profile.json5; unsigned builds cannot install on a device.' }
      }
      if (args.action === 'run') {
        if (cli.ok) {
          const argv = ['run']
        ...
        if (br.exitCode !== 0 || br.timedOut) return { ...outside, backend: 'hvigorw', ok: false, stage: 'build', output: tailText((bOut + '\n' + bErr).trim(), 6000), error: 'hvigorw build failed' }
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'check') {
        ...
        return { outsideSandbox: false, backend: 'devecocli', ok: r.ok, exitCode: r.exitCode, timedOut: r.timedOut, format: jsonOk ? 'json' : 'text', result: json != null ? json : null, output: json != null ? '' : tailText((r.stdout + '\n' + r.stderr).trim(), 8000), error: r.ok ? '' : (r.stderr || r.stdout) }
      }
      return { ok: false, error: 'unknown action: ' + args.action }
    },
  })
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'search') {
        ...
        return { ok: r.ok, backend: 'devecocli', results: json != null ? json : null, output: json != null ? '' : tailText((r.stdout + '\n' + r.stderr).trim(), 6000), error: r.ok ? '' : (r.stderr || r.stdout), hint: 'Use hms_docs action=read with a documentId from the results to get full content.' }
      }
      if (args.action === 'read') {
