...
export function apply(ctx) {
  ...
  if (skillsService && typeof skillsService.register === 'function') {
    disposers.push(skillsService.register({
      name: 'harmonyos-dev',
      description: 'HarmonyOS/ArkTS 开发要点与 dsh-hdc-bridge 设备调试闭环用法。HarmonyOS development essentials and the hdc-bridge device-debug loop.',
      whenToUse: '开发或调试 HarmonyOS/OpenHarmony 应用时：ArkTS 规范、状态管理、常见错误码、真机/模拟器调试闭环。',
      content: '# HarmonyOS 开发要点（hdc-bridge 版）\n\n## 设备调试闭环（会话已装载 hdc_* 工具时）\n1. hdc_ui_dump 查看当前界面文本 → 2. hdc_ui_find 定位控件坐标 → 3. hdc_ui tap/input/swipe 操作 → 4. hdc_ui_dump 验证状态\n- 视觉模型：hdc_screenshot → read_image\n- 失败诊断：hdc_crash（崩溃）、hdc_hilog（日志）、hdc_diag（插件状态）\n\n## ArkTS 要点\n- @Provide/@Consume 必须成对出现，缺失时运行期报 140112\n- 页面路由优先 Navigation/NavPathStack 新范式\n- UI 刷新依赖状态变量（@State/@Prop/@Link 等），直接修改普通属性不会触发渲染\n- 权限在 module.json5 的 requestPermissions 声明，敏感权限需运行时申请\n\n## 常见错误码\n- 140112：@Consume 缺少对应的 @Provide\n- 9568332：签名 profile 未绑定设备 UDID\n- 10002：网络不可达或缺少 INTERNET 权限\n- 401：组件参数不匹配\n\n## 输入技巧\n- uitest 注入混合字符串（数字→字母→数字）时，IME 模式切换会吞掉紧跟字母后的字符：分段输入 + dump 校验 + 缺失字符补发\n- 软键盘改变布局：每次点击前用最新 dump 的坐标；按钮被键盘遮挡时先 key Back 收键盘\n',
    }))
  }
