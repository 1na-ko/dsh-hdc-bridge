  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (action === 'list') {
        ...
        return { ok: r.exitCode === 0, backend: 'devecocli', instances: json != null ? json : null, output: json != null ? '' : tailText(((r.stdout || '') + (r.stderr || '')).trim(), 4000), error: r.exitCode === 0 ? '' : tailText((r.stderr || r.stdout || '').trim(), 1500) }
      }
      if (action === 'start') {
