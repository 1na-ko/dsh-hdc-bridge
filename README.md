...
| 设备面板 | web 宿主浮动面板——已连接设备（型号/API 版本）、一键截图缩略、hilog 尾部、连接指引；数据走 `/api2/hdc-bridge/*` 只读 REST。**v0.7：左下角「鸿蒙」FAB 开关**——面板默认收起不打扰，点击开关面板，可见性/布局/缩放持久化，隐藏时降为 60s 慢轮询（FAB 状态点 + 设备数徽标保持新鲜），headless 宿主自动跳过 |
